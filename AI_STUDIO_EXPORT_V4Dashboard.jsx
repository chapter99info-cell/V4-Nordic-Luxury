...
export const V4Dashboard = () => {
  ...
  const handleOpenWalkIn = (staffId, staffName, time) => {
    ...
    <div className={`min-h-screen ${palette.bg} p-6 font-sans ${palette.text}`}>
      ...
      <header className="flex flex-col md:flex-row justify-between items-center mb-10 gap-6">
        ...
        <button 
          ...
          className="bg-[#D4A373] hover:bg-[#B5895D] text-white px-12 py-5 rounded-full font-black text-xl shadow-[0_15px_30px_-10px_rgba(212,163,115,0.5)] transition-all active:scale-95"
        >
          + WALK-IN NOW
        </button>
      </header>
                      ...
                      {active ? (
                        <div 
                          ...
                          className={`w-full h-full rounded-[1.5rem] border-2 p-4 flex flex-col justify-center items-center cursor-pointer transition-all hover:scale-[1.03] hover:shadow-xl
                            ${active.type === 'cleaning' ? palette.cleaning : (idx % 2 === 0 ? palette.booked.teal : palette.booked.rose)}
                          `}
                        >
